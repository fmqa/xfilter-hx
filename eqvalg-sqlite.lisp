...
(defparameter *default-database* "db.sqlite3")

(defmethod eqvalg-query:distinct-in ((db sqlite:sqlite-handle) column)
  (lambda (where offset limit)
    (mapcar
     #'car
     (sqlite:execute-to-list db (eqvalg-sql:sqlize-distinct column where offset limit)))))

(defmethod eqvalg-query:cardinality-in ((db sqlite:sqlite-handle) term)
  (if (consp term)
      (multiple-value-list
...
(defun call-with-database (thunk)
  (sqlite:with-open-database (eqvalg-query:*store* *default-database*)
    (funcall thunk)))
